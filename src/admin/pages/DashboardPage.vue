...
import { portfolioItems, orders, statusMeta } from '@/admin/data/adminData'
...
const activeServiceCount = computed(
  () => servicesStore.items.filter((s) => s.status === 'active').length,
)
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="bg-white border border-gray-200 p-5 flex flex-col gap-1">
      ...
      <div class="text-3xl font-semibold">
        {{ activeServiceCount }}
        <span class="text-base text-ink/40 font-normal">/ {{ servicesStore.items.length }}</span>
      </div>
    </div>
    <div class="bg-white border border-gray-200 p-5 flex flex-col gap-1">
  ...
  <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-wrap gap-3">
      <RouterLink
        ...
        class="border border-ink text-sm font-semibold px-5 py-2.5 hover:bg-ink hover:text-paper transition-colors"
        >+ Add Portfolio Item</RouterLink
      >
      <RouterLink
        :to="{ name: 'admin-testimonials' }"
        class="border border-ink text-sm font-semibold px-5 py-2.5 hover:bg-ink hover:text-paper transition-colors"
        >+ Add Testimonial</RouterLink
      >
      <RouterLink
  ...
  <div class="flex flex-col gap-3">
    <div class="flex items-center justify-between">
      ...
      <RouterLink
        ...
        class="text-xs font-semibold text-ink/60 hover:text-accent"
        >View All →</RouterLink
      >
    </div>
      ...
      <table class="w-full text-sm min-w-[600px]">
        ...
        <tbody>
          <tr
            ...
            :key="order.id"
            class="card-row border-b border-gray-100 last:border-b-0"
          >
            <td class="px-5 py-3.5">
              <RouterLink
                :to="{ name: 'admin-order-detail', params: { id: order.id } }"
                class="hover:underline"
                >{{ order.name }}</RouterLink
              >
            </td>
            ...
            <td class="px-5 py-3.5">
              <span
                ...
                :class="statusMeta[order.status].badgeClass"
