<script setup>
import { services } from '@/frontend/data/placeholderData'
</script>
...
<template>
  <section class="max-w-6xl mx-auto px-6 py-16 border-t border-ink/10">
    <div class="flex flex-wrap justify-between items-end gap-4 mb-10">
      ...
      <RouterLink :to="{ name: 'services' }" class="text-sm uppercase tracking-wide font-semibold hover:text-accent">All Services →</RouterLink>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      ...
      >
        <div
          ...
          :class="service.status === 'active' ? 'border border-ink text-ink' : 'border border-ink/30 text-ink/50'"
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6">
            ...
            <path v-if="service.icon === 'palette'" d="M12 2a10 10 0 1 0 0 20c1.1 0 2-.9 2-2 0-.5-.2-1-.5-1.3-.3-.4-.5-.8-.5-1.3 0-1.1.9-2 2-2h2.3A5.2 5.2 0 0 0 22 12 10 10 0 0 0 12 2z"/>
            ...
            <path v-if="service.icon === 'tag'" d="M20.59 13.41 13.42 20.6a2 2 0 0 1-2.83 0L3 13V3h10l7.59 7.59a2 2 0 0 1 0 2.82z"/>
            <circle v-if="service.icon === 'tag'" cx="7.5" cy="7.5" r="1.5"/>
            <path v-if="service.icon === 'tshirt'" d="M8 3L4 6l2 3 2-1v11h8V8l2 1 2-3-4-3-2 1a2 2 0 0 1-4 0z"/>
          </svg>
        </div>
        <span
        ...
        <p class="text-sm flex-1" :class="service.status === 'active' ? 'text-ink/60' : 'text-ink/50'">{{ service.description }}</p>
      </div>
    </div>
  </section>
</template>
